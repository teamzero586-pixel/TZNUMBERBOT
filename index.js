...
// NUMBER MASKING FUNCTION (Khoobsurat display ke liye)
function maskNumber(numStr) {
    const str = String(numStr).trim();
    if (str.length <= 6) return str; // Agar number buhat chota hai tou as it is show kare
    // Shuru ke 4 hisse aur end ke 2 hisse
        ...
        if (Array.isArray(items)) {
            ...
            for (let item of items.reverse()) {
                ...
                if (!lastSeenOtpIds.has(uniqueId)) {
                    ...
                    // Yahan par hum masked number use kar rahe hain
                    const displayNum = maskNumber(phoneNumber);

                    // OTP text mein Number wala hissa add kar diya gaya hai
