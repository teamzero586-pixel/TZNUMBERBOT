...
const API_KEY = process.env.API_KEY || "np_live_yltQxyzf5AruC7F-jTYZS82NTse7hq2VwXMVVrM-4vs"; // Updated API Key
const POLL_INTERVAL = 2000; // Fast checking (2 seconds)
...
async function requestApiNumber(service, country) {
    try {
        ...
        console.error(`API Request Number Error (${service} - ${country}):`, err.response ? err.response.data : err.message);
    }
    return null;
}

// --- LIVE TRAFFIC FETCH FUNCTION ---
async function getLiveTrafficText() {
    try {
        ...
        if (otpRes && Array.isArray(otpRes.data) && otpRes.data.length > 0) {
            ...
            text += `\n💡 *Tip:* Select high-traffic services above for instant OTP delivery!`;
        } else {
...
async function sendMainMenu(chatId, messageId = null) {
    ...
    // 🔥 LIVE TRAFFIC BUTTON ADDED HERE IN MAIN MENU
...
bot.on('callback_query', async (query) => {
    ...
    // FIXED SERVICE SELECTION USING PIPES '|'
    if (data.startsWith('srv|') || data.startsWith('srv_')) {
        ...
        // 1. Fetch countries from API specifically for this service
        ...
        if (countryListMap.size === 0) {
            const defaultCountries = ['Indonesia', 'United States', 'Pakistan', 'India', 'Russia', 'United Kingdom', 'Germany', 'Laos', 'Malaysia'];
        ...
        countryListMap.forEach((count, cName) => {
            ...
            // Safe delimiter '|' prevents country name splitting issues
            kbButtons.push([{ text: `${flag} ${cName} (${count})`, callback_data: `cnt|${sName}|${cName}` }]);
        });
    ...
    // FIXED COUNTRY SELECTION USING PIPES '|'
...
async function sendBatchNumbers(chatId, service, country, messageId) {
    ...
    // 1. Check MongoDB for manually uploaded numbers under EXACT service & country
    ...
    // 2. If MongoDB is empty, fetch automatically from NumberPanel API!
    ...
    // 3. Fallback single retry if parallel batch returned empty
    if (numbersToDeliver.length === 0) {
