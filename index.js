...
const POLL_INTERVAL = 3000; // Ultra fast checking (3 seconds)
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('copy_otp_')) {
        ...
        return bot.answerCallbackQuery(query.id, {
            ...
            show_alert: true // Shows a nice popup with the copied code
        }).catch(() => {});
    }
...
// --- REAL-TIME API POLLING (PREMIUM LAYOUT & COPY BUTTON) ---
        ...
        if (Array.isArray(items)) {
            ...
            for (let item of items.reverse()) {
                ...
                if (!lastSeenOtpIds.has(uniqueId)) {
                    ...
                    // PRECISE & BEAUTIFUL SCREENSHOT MATCHING TEXT FORMAT
                    const text = `🔥 *TEAM ZERO OTP RECEIVED* 🔥\n\n🌐 Service: *${service}*\n${countryInfo.flag} Country: *${countryInfo.name}*\n💬 OTP Code: \`${otpCode}\`\n\n_POWERED BY TEAM ZERO_`;
                    
                    // SCREENSHOT MATCHING LAYOUT (COPY BUTTON + 3 SIDE-BY-SIDE BUTTONS)
                    const markup = {
                        inline_keyboard: [
