...
console.log("🚀 TEAM ZERO OTP Bot is running...");
...
async function checkForceJoin(userId) {
    ...
    try {
        ...
        const isChannelJoined = validStatuses.includes(channelCheck.status);
        const isGroupJoined = validStatuses.includes(groupCheck.status);
        
        return isChannelJoined && isGroupJoined;
    } catch (error) {
        console.error("Force Join Check Error:", error.message);
        // Agar bot channel/group mein admin nahi hoga to error aayega, isliye False return karein taake force join lazmi ho
        return false; 
    }
}
...
⚠️ *Note:* Bot use karne ke liye pehle niche diye gaye Telegram Channel aur Group ko join karna lazmi hai! Join karne ke baad **Verify** button par click karein.
...
Neeche diye gaye buttons se apni service select karein taake live numbers aur OTPs show ho sakein.
...
bot.on('callback_query', async (query) => {
    ...
    // Strict Force Join Check for all clicks
    const isJoined = await checkForceJoin(chatId);
    if (!isJoined) {
        bot.answerCallbackQuery(query.id, { text: "⚠️ Pehle Channel aur Group join karein!", show_alert: true });
        bot.deleteMessage(chatId, query.message.message_id).catch(() => {});
        return sendForceJoinMenu(chatId);
    }
    ...
    if (data === 'admin_panel') {
        ...
        bot.answerCallbackQuery(query.id, { text: `Fetching ${sName.toUpperCase()} numbers...` });
        
        try {
            const response = await axios.get(API_URL);
            const items = response.data;
            if (Array.isArray(items)) {
                // Filter items matching the requested service
                const filtered = items.filter(item => String(item[0]).toLowerCase().includes(sName)).slice(0, 5);
                
                if (filtered.length === 0) {
                    bot.sendMessage(chatId, `⚠️ Filhal *${sName.toUpperCase()}* ke liye koi active number available nahi hai. Thori der baad try karein.`);
                } else {
                    let replyText = `📱 *Latest ${sName.toUpperCase()} Numbers & OTPs:*\n\n`;
                    filtered.forEach((item, index) => {
                        const num = item[1] || 'Unknown';
                        const msgText = item[2] || '';
                        const otp = extractOtp(msgText);
                        replyText += `${index + 1}️⃣ Number: \`${num}\`\n🔑 OTP: \`${otp}\`\n\n`;
                    });
                    replyText += `_POWERED BY TEAM ZERO_`;
                    bot.sendMessage(chatId, replyText, { parse_mode: 'Markdown' });
                }
            }
        } catch (e) {
            bot.sendMessage(chatId, "❌ Numbers fetch karne mein error aa gaya. Dobara koshish karein.");
        }
    }
...
// --- BACKGROUND OTP POLLING POOL (Group Forwarder) ---
