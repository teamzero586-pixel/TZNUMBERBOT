...
const API_KEY = process.env.API_KEY || "np_live_jTYZS824vs"; // API Authorization Bearer Key
const POLL_INTERVAL = 2000; // Ultra fast checking (2 seconds)
...
function getCountryFlagByName(countryName) {
    const found = countryDatabase.find(c => c.name.toLowerCase() === countryName.toLowerCase());
...
async function fetchApiServices() {
    try {
        ...
        if (res.data && res.data.success && Array.isArray(res.data.services)) {
            return res.data.services.map(s => s.name);
        }
    } catch (err) {
...
async function fetchApiCountries(serviceName) {
    try {
        ...
        if (res.data && res.data.success && Array.isArray(res.data.countries)) {
            return res.data.countries; // Array of { name: '...', count: ... }
        }
    } catch (err) {
...
async function requestApiNumber(service, country) {
    try {
        ...
        console.error("API Request Number Error:", err.response ? err.response.data : err.message);
    }
    return null;
}
...
async function sendMainMenu(chatId, messageId = null) {
    // Combine API services with Database services automatically
    ...
    const combined = [...new Set([...defaultServices, ...apiServices.map(s => s.toLowerCase()), ...dbServices.map(s => s.toLowerCase())])];
    ...
    const welcomeMessage = `Welcome to *TEAM ZERO OTP BOT* 🚀\n\nNeeche di gayi services mein se apni pasand ki service select karein (All numbers generated automatically):\n\n_POWERED BY TEAM ZERO_`;
...
// --- CALLBACK QUERIES (ULTRA FAST & AUTO NUMBERS) ---
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('srv_')) {
        ...
        // Show loading state while fetching dynamic countries
        ...
        if (countryListMap.size === 0) {
            // Default fallback popular countries if API has no specific filter response
            const defaultCountries = ['Indonesia', 'United States', 'Pakistan', 'India', 'Russia', 'United Kingdom', 'Germany'];
            defaultCountries.forEach(cnt => countryListMap.set(cnt, 'Auto'));
        }

        let kbButtons = [];
        countryListMap.forEach((count, cName) => {
            let flag = getCountryFlagByName(cName);
            kbButtons.push([{ text: `${flag} ${cName} (${count})`, callback_data: `country_${sName}_${cName}` }]);
        });
...
async function sendBatchNumbers(chatId, service, country, messageId) {
    ...
    // 1. Check MongoDB first for manually uploaded numbers
    ...
    // 2. If MongoDB has no numbers, fetch automatically from NumberPanel API!
    if (numbersToDeliver.length === 0) {
        let apiPromises = [];
        for (let i = 0; i < 5; i++) {
            apiPromises.push(requestApiNumber(service, country));
        }

        const results = await Promise.all(apiPromises);
        numbersToDeliver = results.filter(n => n !== null);
    }

    // 3. Fallback if API or DB didn't return numbers (Retry single API request or show empty message)
    if (numbersToDeliver.length === 0) {
